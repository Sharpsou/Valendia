...
FULL_FBX_DIR = PROJECT / "Assets" / "Valendia" / "Art" / "Environment" / "Trees" / "FullFBX"
...
def export_tree(source_name):
    clear_scene()
    source_path = SOURCE_FBX_DIR / f"{source_name}.fbx"
    target_path = FULL_FBX_DIR / f"{source_name}_full.fbx"
    ...
    for index, (material_name, objects) in enumerate(sorted(groups.items())):
        ...
        full_objects.append(join_objects(objects, f"{source_name}_{index:02d}_{safe_material_name}"))

    for obj in list(bpy.context.scene.objects):
        if obj.type != "MESH":
            bpy.data.objects.remove(obj, do_unlink=True)

    after = mesh_stats(full_objects)
    FULL_FBX_DIR.mkdir(parents=True, exist_ok=True)
